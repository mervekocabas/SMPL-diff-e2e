...
class Unetbackbone(nn.Module):
    def __init__(self, hparams=None, precision=torch.float32, img_res=224,):
        ...
        self.register_buffer('init_pose_6d', init_pose_6d)
    ...
    def forward(
        ...
        noise_pred = self.denoise_forward(clean_rgb_noisy_smplx_latents, timesteps)
        pred_smplx_params = self.get_pred_smplx_from_noise_pred(noise_pred, noisy_smplx_latents, timesteps)
    ...
    def encode_smplx_params(self, pose_aa, shape, cam, rgb_latents):

        batch_size = rgb_latents.shape[0]
        smplx_latents = torch.zeros_like(rgb_latents[:, :1]).reshape(batch_size, -1)
        pose_6d = axis_angle_to_rotation_6d(pose_aa.reshape(-1, 3)).reshape(batch_size, -1)
        
        # subtract mean pose
        res_pose_6d = pose_6d - self.init_pose_6d
        res_shape = shape - self.init_shape
        res_cam = cam - self.init_cam
        
        smplx_params_cat = torch.cat([res_pose_6d, res_shape, res_cam], dim=-1)
        smplx_latents[:, :smplx_params_cat.shape[-1]] = smplx_params_cat
        smplx_latents = smplx_latents.reshape(*rgb_latents[:, :1].shape)
        return smplx_latents
